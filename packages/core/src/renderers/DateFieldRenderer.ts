...
import { DateFieldConfig, Renderer } from '../types';

export const RenderDateField: Renderer<string, DateFieldConfig> = (
  container,
  field,
  ...
  if (field.config.min) input.min = String(field.config.min);
  if (field.config.max) input.max = String(field.config.max);
