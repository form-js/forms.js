...
import { FileFieldConfig, Renderer } from '../types';

export const RenderFileField: Renderer<FileList | null, FileFieldConfig> = (
  container,
  field,
  ...
  input.oninput = (e) => field.setValue((e.target as HTMLInputElement).files);
  if (field.config.multiple === true) input.multiple = field.config.multiple;
  if (field.config.accept != null) input.accept = field.config.accept;
