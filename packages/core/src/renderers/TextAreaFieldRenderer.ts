...
import { Renderer, TextAreaFieldConfig } from '../types';

export const RenderTextAreaField: Renderer<string, TextAreaFieldConfig> = (
  container,
  field,
  ...
  if (field.config.minLength != null) input.minLength = field.config.minLength;
  if (field.config.placeholder != null) input.placeholder = field.config.placeholder;
  input.rows = field.config.rows || 5;
