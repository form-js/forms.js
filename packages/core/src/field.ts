...
import { AsyncValidator, Renderer, FieldConfig, FieldState } from './types';
...
export class Field<T, C extends FieldConfig<T>> {
  ...
  private type: FieldTypes | undefined = undefined;
  ...
  constructor(
    ...
    if (config.useDefaultValidators !== false) {
      this.config$.validators?.push(requiredValidator);
    }
  ...
  private runValidators(value: T | null): Observable<string[]> {
    ...
    const asyncValidators: AsyncValidator<T, FieldConfig<T>>[] = [];

    for (const validator of validators) {
      try {
        const result = validator(value, this.fieldState());
    ...
    if (asyncValidators.length > 0) {
      const asyncValidationPromises = asyncValidators.map((validator) =>
        validator(value, this.fieldState())
          .then((result: string | string[] | null) => result || [])
          .catch(() => []),
      );
