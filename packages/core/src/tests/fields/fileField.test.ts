...
import { createForm, baseFileFieldTestOptions, FILE_FIELD_ID, DEFAULT_FILE_VALUE, INVALID_EMAIL_VALUE, DEFAULT_STRING_VALUE } from './../test.options';
...
describe('file-field', () => {
    ...
    it('input triggers change event', () => {
        ...
        const field = form.getField(FILE_FIELD_ID)! as unknown as FileField;
        const event = { target: { files: DEFAULT_STRING_VALUE } } as unknown as HTMLElementEvent<HTMLInputElement> & { files: FileList };
        field.change(event);
        expect(mockChange).toHaveBeenCalledWith(DEFAULT_STRING_VALUE);
    });

    it('enhances file input to filepond', () => {
        const form = createForm({
            schema: [
                {
                    ...baseFileFieldTestOptions,
