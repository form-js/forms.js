...
<iframe height="300" style="width: 100%;" scrolling="no" title="forms.js - telephone example" src="https://codepen.io/trilmatic/embed/LYqvBXa?default-tab=js%2Cresult" frameborder="no" loading="lazy" allowtransparency="true" allowfullscreen="true">
  See the Pen <a href="https://codepen.io/trilmatic/pen/LYqvBXa">
  forms.js - telephone example</a> by Trilmatic (<a href="https://codepen.io/trilmatic">@trilmatic</a>)
...
```js
public telFormat = /^+?(\d{1,3})?[-.\s]?(?\d{1,4}?)?[-.\s]?\d{1,4}[-.\s]?\d{1,4}[-.\s]?\d{1,9}$/;
